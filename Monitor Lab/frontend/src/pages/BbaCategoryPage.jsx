...
import { getBbaCategoryBySlug } from "../data/bbaCategories";

function BbaCategoryPage() {
  const { categorySlug } = useParams();
  const category = getBbaCategoryBySlug(categorySlug);
    ...
    return (
      <div className="bca-container bba-bg">
        ...
        <div className="bca-cards">
          <Link to="/bba" className="bca-card-link">← Back to BBA Curriculum</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="bca-container bba-bg">
      <header className="bca-cat-header">
        <div className="bca-cat-header-top">
          <Link to="/bba" className="back-btn">&larr; Back</Link>
          <h1>{category.name}</h1>
        </div>
        <p>{category.desc}</p>
      </header>
...
export default BbaCategoryPage;
